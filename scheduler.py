...
def createDepenGraph(fileIn):
    ...
    for index, originalLine in enumerate(fileIn):
        ...
        if instruction == "load":
            ...
            # Dont allow store to cross load
            if "store" in write:
                temp = write["store"].line
                nodes[temp].addChild(index, curr)
                curr.addParent(temp, nodes[temp])
            if "storeAI" in write:
                temp = write["storeAI"].line
                nodes[temp].addChild(index, curr)
                curr.addParent(temp, nodes[temp])
            if "storeAO" in write:
                temp = write["storeAO"].line
                nodes[temp].addChild(index, curr)
                curr.addParent(temp, nodes[temp])
        ...
        if instruction == "store":
            ...
            # Dont allow load and store to cross
            if "load" in read:
                temp = read["load"].line
                nodes[temp].addChild(index, curr)
                curr.addParent(temp, nodes[temp])
            if "loadAI" in read:
                temp = read["loadAI"].line
                nodes[temp].addChild(index, curr)
                curr.addParent(temp, nodes[temp])
            if "loadAO" in read:
                temp = read["loadAO"].line
                nodes[temp].addChild(index, curr)
                curr.addParent(temp, nodes[temp])
        ...
        if instruction == "storeAI":
            ...
            if param2 + imm in read:
                temp = read[param2 + imm].line
                nodes[temp].addChild(index, curr)
                curr.addParent(temp, nodes[temp])
...
def a(depGraph):
    ...
    #for i in depGraph:
    #    print(str(i.line) + " lat: " + str(i.latencyPath))
...
def countLatencies(graph):
    ...
    for bot in bottom:
        graph[graph.index(bot)].latencyPath = latency[graph[graph.index(bot)].inst]
